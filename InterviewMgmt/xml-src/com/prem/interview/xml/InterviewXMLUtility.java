...
import com.prem.interview.rpc.InterviewRPC;
import com.prem.interview.xml.ProjectConfigConstants.Answer;
import com.prem.interview.xml.ProjectConfigConstants.Category;
import com.prem.interview.xml.ProjectConfigConstants.Question;
import com.prem.interview.xml.ProjectConfigConstants.Type;
...
public class InterviewXMLUtility {
	...
	public InterviewXMLUtility(String xmlFileName)
...
//
//			Element categoryQuestionListElement = createCategoryQuestionsList(categoryId);
//
...
//			/** ################ ANSWERS START ####################### */
//
//			Element categoryQuestionAnswerListElement = fetchAllAnswersByQuestion(cats);
//
...
//	}
	...
	public List<CategoryDTO> fetchAllCategories()
		...
		NodeList nl = utility.getXpathNodeListResult(utility.getDom(), "/"
				...
				+ ProjectConfigConstants.CATEGORY_LIST.getName() + "/"
				+ Category.node.getName());
		for (int index = 0; index < nl.getLength(); index++) {
			Node categoryNode = nl.item(index);
			CategoryDTO cat = new CategoryDTO();
			cat.setCatID(utility.getXpathIntegerResult(categoryNode,
					Category.ID.getName()));
			cat.setCatgoryName(utility.getXpathStringResult(categoryNode,
					Category.CATEGORY_NAME.getName()));
	...
	public List<CategoryDTO> fetchAllCategoriesByName(String name,StringComparison sc)
		...
		NodeList nl = utility.getXpathNodeListResult(utility.getDom(), "/"
				...
				sc.getName() +
						"("
				+ Category.CATEGORY_NAME.getName() + ",'" + name + "')]");
		for (int index = 0; index < nl.getLength(); index++) {
			Node categoryNode = nl.item(index);
			CategoryDTO cat = new CategoryDTO();
			cat.setCatID(utility.getXpathIntegerResult(categoryNode,
	...
	public CategoryDTO findCategoryByID(int CatID)
		...
		NodeList nl = utility.getXpathNodeListResult(utility.getDom(), "/"
				+ ProjectConfigConstants.ROOT.getName() + "/"
				+ ProjectConfigConstants.CATEGORY_LIST.getName() + "/"
				+ Category.node.getName() + "[" + Category.ID.getName() + "="
				+ CatID + "]");
		if (nl != null && nl.getLength() > 0) {
			System.out.println("Total result found == " + nl.getLength());
			Node categoryNode = nl.item(0);
			objCategoryDTO = new CategoryDTO();
			objCategoryDTO.setCatID(utility.getXpathIntegerResult(categoryNode,
					Category.ID.getName()));
			objCategoryDTO.setCatgoryName(utility.getXpathStringResult(
					categoryNode, Category.CATEGORY_NAME.getName()));
		}
	...
	public static void main(String[] args) {
		...
		try {
			InterviewXMLUtility inter = new InterviewXMLUtility(
					"C:/Users/VINU/Desktop/services.xml");
			// inter.refreshXML();
