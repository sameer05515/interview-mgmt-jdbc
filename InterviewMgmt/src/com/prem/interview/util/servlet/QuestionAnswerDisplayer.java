	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			out.println(new StringBuffer()
					.append("<html>")
					.append("<title>QuestionAnswerDisplayer</title>"
			...
			for (CategoryDTO cats : listCats) {
				...
				for (QuestionDTO q : listques) {
					...
					sbbb.append("<li>")
							.append("<a href=\"#" + cats.getCatID() + "_"
									+ q.getQuestionID() + "\">")
							.append(q.getQuestion().toUpperCase())
							.append("</a>");
					sbbb.append("</li><br>");
			...
			for (CategoryDTO cats : listCats) {
				...
				/* "<center>" + */
				"#################################" + "<br>"
						+ "<font size=\"5\">" + "<a id=\"" + cats.getCatID()
						+ "\">" + " category : ".toUpperCase() + (catCounter++)
						+ "</a>" + "<br>" + cats.getCatgoryName().toUpperCase()
						+ "</font><br>" + "#################################"
						+ "<br>");
				Vector<QuestionDTO> listques = intMgmtRPC
				...
				for (QuestionDTO q : listques) {
					...
					for (AnswerDTO ans : listAns) {
						out.print("<br><font size=\"3\" style=\"font-weight: bold\" color=\"green\" > { ANSWER : "
								+ answerNo
								+ " } </font><br><font size=\"3\" style=\"font-weight: bold\">"
								+ ans.getAnswer() + "</font>");
					}
					out.print("<br>**********<br>");
				}
